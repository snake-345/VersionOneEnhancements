KangoAPI.onReady(function() {
	...
	kango.dispatchMessage('editReady');
	kango.addMessageListener('sendCaptured', function(event) {
	...
	function initForm() {
		kango.xhr.send({
			method: 'GET',
			url: 'https://www8.v1host.com/DealerSocket/rest-1.v1/Data/Scope?sel=Name,Parent&where=AssetState="64"&Accept=application/json',
			contentType: 'json',
			headers: {
				'Authorization': 'Bearer 1.0yzDvxnaLXDakgISQ46GqST0pj4='
			}
		}, function (data) {
			fieldsData.scopes = getTree(data.response, null, function(asset) {
				return {
					id: asset.id
				}
			});
		...
		kango.xhr.send({
			method: 'GET',
			url: 'https://www8.v1host.com/DealerSocket/rest-1.v1/Data/Theme?sel=Name,Scope,Parent&where=AssetState="64"&Accept=application/json',
			contentType: 'json',
			headers: {
				'Authorization': 'Bearer 1.0yzDvxnaLXDakgISQ46GqST0pj4='
			}
		}, function (data) {
			fieldsData.parents = getTree(data.response, null, function(asset) {
				return {
					id: asset.id,
		...
		document.querySelector('form').addEventListener('submit', function(event) {
			...
			var image = '<img src="' + canvas.toDataURL() + '" alt="screenshot" style="max-width: 100%">';
			...
			kango.xhr.send(options, function (data) {
				var defect = data.response.querySelector('Asset').getAttribute('id').split(':');
				...
				kango.dispatchMessage('openDefectEditing', url);
			});
