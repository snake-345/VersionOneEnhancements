 ...
 * Copyright (C) 2012-15 Alfonso Martínez de Lizarrondo
 *
...
(function() {
	...
	function showMessage(editor, msg) {
		...
		if (!dialog && editor.showNotification)
	...
	function PickAndSendFile(editor, forImage, caller, callback) {
		...
		filePicker.accept = forImage ? '.jpg, .png, .jpeg, .gif, .bmp|images/*' : '';
	...
	*/
	function receivedUrl(fileUrl, data, editor, el, attribute) {
		if (el.$.nodeName.toLowerCase() == 'span') {
			...
			if (data.originalNode) {
				...
				var original = data.originalNode;
				...
				var throbber = {
					...
					center : function() {
						var mainThrobber = this.mainThrobber,
							cover = this.dialog.throbberCover;
						// Center the throbber
			...
			if (typeof evtData.url == 'undefined') {
				...
				if (!fileUrl && !msg) {
					...
					if (!parts) {
						...
						if (window.console)
							console.log(xhr.responseText); // eslint-disable-line no-console
					}
				}
	...
	function createSimpleUpload( editor, dialogName, definition, element ) {
		...
		var filebrowser = element.filebrowser,
			forImage = dialogName.substr(0,5) == 'image' || filebrowser.requiresImage,
			targetField = filebrowser.target && filebrowser.target.split(':');
		...
		var callback = {
			...
			upload : function(url, msg, data) {
				...
				if (filebrowser.onSelect) {
					if ( filebrowser.onSelect( url, msg, data ) === false )
						return;
				}
